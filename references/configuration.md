...
2. JSON config file from `MINA_WIKI_CONFIG`
3. nearest `.mina-wiki.json` found by walking from the current working directory toward `/`
...
Use shell-profile setup when the same raw/wiki roots should apply across many sessions. Use `.mina-wiki.json` when a specific project should override the default roots.

## Sync metadata

The skill stores sync metadata in:

`WIKI_DIR/.mina-wiki/last_sync.json`
...
The skill may also keep sync bookkeeping such as source-to-page mappings under `WIKI_DIR/.mina-wiki/`. That metadata supports maintenance workflows and should not be treated as part of the user-facing wiki content model.
